...
export default function Logo({ className }: LogoPropTypes) {
  ...
  return (
    isMounted && (
      ...
      >
        <span className="relative flex overflow-hidden mt-5 ml-4">
          {isDarkMode && (
            <Image src={darkLogo} alt="Criptic" height={55} priority />
          )}
          {!isDarkMode && (
            <Image src={lightLogo} alt="Criptic" height={55} priority />
          )}
        </span>
      </AnchorLink>
    )
